    ...
    initSubmit　: function(){
		...
		el.find('.pageback').off('touchend').on("touchend",function(evt){
			...
			if(!$('.order-list') || $('.order-list > li').length ==0){
        		...
        		el.find('.orderTotal .trace input').val(1);
			}else{
				var tip = {
		    			type : 'dialog',
		    			title : '返回',
    	...
    	el.find('.addOrder span').on('touchend',function(evt){
    		...
    		lt.find('.subTab span.tabOn').trigger('tap');
    	});
...
//    		
    		...
    		$('#orderDetail .orderdetail>div>div:nth-child(2)').html('');
...
//					if(pullUpEl.hasClass('second-pull')){
//						pullUpEl.show();
//					}else{
//						pullUpEl.hide();
//					}
    	...
    	scroll.on('scrollEnd',function(){
			if(me.loadingStatus == 1 && !me.loading){
				...
				if(pullDownEl.hasClass('flip')){
					...
					if(!pullUpEl.hasClass('second-pull')){
						pullUpEl.addClass('second-pull').removeClass('flip');
						me.loadingStatus = 0;
						me.loading = false;
						var y = Common.scroll[0].y + pullDownEl.height();
						//Common.scroll[0].scrollTo(0,y,500);
					}else{
						...
						setTimeout(function(){
							...
							pullUpEl.removeClass('second-pull');
						},1000);
					}
				}
			}
				        		...
				        		Api.stopTrace(obj,function(res){
				        			if(res.code == '1'&&res.result.length>0){
				        				...
				        				$(res.result).each(function(i,v){
				        					$('#'+v.orderId).removeClass('on').addClass('disable');
    ...
    initMyAccount : function(){
    	var me = this;
    	$('#index .feature').on('tap',function(evt){
    		evt.preventDefault();
    	...
    	//转至充值页面
    	$('#index .recharge-btn').on('tap',function(evt){
    		me.rechargeIndex();
    		Common.pageIn('#rechargeMoney');
    	});
    	
    	//转至提现页面
    	$('#index .withdraw-btn').on('tap',function(evt){
    		me.getWithDrawInfo();
    	});
    	...
    	$('#accountPage .logout').on('tap',function() {
			var tip = {
				type : 'dialog',
				title : '退出登录',
				content : '确定要退出登录吗？',
    ...
    recharge : function(){
    	var me = this;
    	//Common.showLoading();
    	var backId = $('#rechargeMoney .rechargeBank').val();
...
Common = {
	...
	initDevice : function(){
		...
		os.isSymbian = /(?:symbianos)/.test(ua) || os.isWindowsPhone;
		os.isTablet = /(?:ipad|playbook)/.test(ua)||(os.isFireFox && /(?:tablet)/.test(ua));
		os.isPc = !os.isPhone && !os.isAndroid && !os.isSymbian;
		os.isPhone = /(?:iphone)/.test(ua) && !os.isTablet;
		
		var IOS_Safari = (os.isIpad || os.isPhone) && !os.isChrome && os.isSafari && !os.isQB && !os.isFireFox && !os.isUC;
		var Android_Chrome = os.isAndroid && os.isChrome && !os.isQB && !os.isFireFox && !os.isUC;
		
		if(IOS_Safari){
	  		var content = '请点击下方的<i class="icon1"></i>,然后点击<i class="icon2"></i>即可将摩臣娱乐添加到桌面';
	  		$('#deviceMask .deviceTip').addClass('safari').find('.content').html(content);
	  		$('#deviceMask').show();
	  	}else if(Android_Chrome){
	  		var content = '请点击右上方的<i class="icon1"></i>,然后选择添加到主屏幕即可将摩臣娱乐添加到桌面';
	  		$('#deviceMask .deviceTip').addClass('chrome').find('.content').html(content);
	  		$('#deviceMask').show();
	  	}
		
		$('#deviceMask').on('touchend',function(){
        	$(this).hide();
        })
	},
	...
	initWrapper : function(){
		...
		if(wrapper){
			...
			var o = me.scrollObj[me.currentPage];
			if(o){
				me.scroll[0].scrollTo(0,o.y,0);
				me.scrollObj = {};
			}
		}
	},
